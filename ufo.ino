...
bool LoadConfig() {
  ...
  ruxitUrl =(const char*)json["ruxit-url"];
...
void apiHandler() {
  ...
  if (httpServer.hasArg("ruxit-url")) {
    if (debug) Serial.println("Storing ruxit integration settings");  
    ruxitUrl = httpServer.arg("ruxit-url");
    //ruxitApiKey = httpServer.arg("ruxit-apikey");
    if (debug) Serial.println("Stored: " + httpServer.arg("ruxit-url")); // + " / " + httpServer.arg("ruxit-apikey"));
...
void pollRuxit() {
  ...
  if (ruxitUrl.startsWith("https")) {
    if (debug) Serial.println("Calling HTTPS methods will result in out-of-memory crash!");
    return;
  }
  ...
  // The REST call to "https://<environmentid>.live.ruxit.com/api/v1/problem/status?Api-Token=<apikey>" must be
  // a) an HTTP call in case of a local Ruxit Managed deployment or
  // b) use an HTTP to HTTPS proxy such as e.g. AWS API Gateway
  // the resulting JSON must look like this: {"result":{"totalOpenProblemsCount":1,"openProblemCounts":{"INFRASTRUCTURE":1,"SERVICE":0,"APPLICATION":0}}}
  http.begin(ruxitUrl); 
  ...
  if(httpCode == HTTP_CODE_OK) {
      ...
      if (debug) Serial.println("Ruxit Problem API call JSON response: " + json);
...
void WiFiEvent(WiFiEvent_t event) {
    switch(event) {
        ...
        case WIFI_EVENT_STAMODE_DISCONNECTED:
            if (debug) Serial.println("WiFi client lost connection");
            wifiStationOK = false;            
            break;
        case WIFI_EVENT_STAMODE_CONNECTED:
            if (debug) Serial.println("WiFi client connected");
            break;
        case WIFI_EVENT_STAMODE_AUTHMODE_CHANGE:
            if (debug) Serial.println("WiFi client authentication mode changed.");
            break;
...
void setup ( void ) {
  ...
  if (LoadConfig()) {
    if (debug) Serial.println("Ruxit envid: " + ruxitUrl);
  } else {
